let allTestFiles = [];
let TEST_REGEXP = /^\/base\/test\/unit\/[^\/]+\.js$/i;

// Get a list of all the test files to include
Object.keys(window.__karma__.files).forEach(function (file) {
  if (TEST_REGEXP.test(file) && file !== '/base/test/setup.js') {
...
require.config({
  ...
  callback: function (pal) {
    if (started) {
      return;
    }
    started = true;
    pal.initialize();
    window.require(allTestFiles, function () { return window.__karma__.start(); });
  },
  paths: {
